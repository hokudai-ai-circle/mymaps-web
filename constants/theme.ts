...
/**
 * 雰囲気タグ（初見殺し対策。内容ではなく「場の温度感」を伝える）。
 *
 * ⚠️ **実データでは使っていない。** 公式サイトに相当する情報が無く、
 * 実在のセッションにこちらの主観でラベルを貼ることになるため（2026-08-09決定）。
 * 型と描画は残してあるので、公式に相当する情報が出れば入れられる。
 * 詳細は `data/event.ts` の冒頭を参照。
 */
export const MOOD_TAGS = ['がっつり系', 'ゆるい', '初心者歓迎', '学生多め'] as const;
