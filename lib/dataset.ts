...
export type Venue = {
  ...
  /**
   * その会場の位置や徒歩時間について、**利用者に断っておくこと**（#97）。
   *
   * 🔴 **点で表せない会場のためにある。** さっぽろホコテンは
   * 札幌駅前通の南大通〜南4条が会場で、**通りそのもの**。
   * 代表点を1つ置く以上、地図の位置も徒歩時間も端で最大3分ずれる。
   * **黙ってずれた数字を出すより、ずれると書くほうがよい。**
   *
   * ⚠️ **会場の紹介文（desc）とは別。** desc は「何がある場所か」で、
